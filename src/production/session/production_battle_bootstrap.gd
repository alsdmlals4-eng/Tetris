...
func build_runtime() -> Dictionary:
	...
	var runtime = load("res://src/production/runtime/production_combat_runtime.gd").new(player, enemy, workspace, scheduler, skill, pause, response)
	...
	return {"ready": true, "runtime": runtime, "workspace_manager": workspace, "pause_controller": pause, "player": player, "enemy": enemy}

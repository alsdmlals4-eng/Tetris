...
func _drain_board()->Array:
    ...
    for safety in 256:
        ...
        if not _pending.is_empty():
            ...
            if pending.target_board=="LINE":result=line.destroy_cells(pending.action_id,pending.target_ids)
            else:
            ...
            if not result.is_empty():
                ...
                changed=true
        if combat.outcome=="RUNNING" and mode=="LINE" and line.spawn_blocked():
            events.append_array(_line_topout())
            changed=true
        elif combat.outcome=="RUNNING" and mode=="LINE" and line.grounded() and line.grounded_time()>=Line.LOCK_US:
        ...
        if (mode=="CHAIN" or chain.is_resolving()) and chain.next_event_us()==0:
            ...
            if not plan.is_empty():
                ...
                changed=true
        if not chain.is_resolving() and not queued_workspace.is_empty():
...
func _reserve_threat()->void:
    ...
    var candidates:Array=line.target_candidates() if preview.target_board=="LINE" else chain.target_candidates()
    disruption.reserve(combat.eta_us,candidates)

func can_checkpoint()->bool:return not chain.is_resolving() and _pending.is_empty() and queued_workspace.is_empty()

func snapshot()->Dictionary:
    if not can_checkpoint():return {}
    return {"schema":"r3-session-v1","rules_hash":FileAccess.get_sha256(Supply.RULES_PATH),"seed":str(_seed),"run_id":_run_id,
...
func restore(data:Dictionary)->bool:
    if data.size()!=19 or data.get("schema")!="r3-session-v1" or data.get("rules_hash")!=FileAccess.get_sha256(Supply.RULES_PATH):return false
    ...
    if not _valid_ledgers(data):return false
...
func _valid_ledgers(data:Dictionary)->bool:
    ...
    if int(data.metrics.line_clears)*10!=data.supply.cells.size() or int(data.metrics.supply_overflow)!=int(data.supply.discarded):return false
    ...
    for event in data.destruction_events:
        ...
        var processed:Array=data.line.destructions if event.target_board=="LINE" else data.chain.processed_event_ids

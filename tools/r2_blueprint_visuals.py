...
class Screen(Flowable):
    ...
    def paint(self,c):
        ...
        self.text(c,661,365,'현재 · 균열 강타',17);a.paint(c,'R1-ICONS','heavy',661,321,30,30);self.text(c,699,328,'직접 피해 35',17);self.text(c,661,294,'방벽 적용 가능',16,b.CYAN)
        self.text(c,883,369,'공유 타이머',17,b.GOLD)
        c.setStrokeColor(b.CYAN);c.setLineWidth(4);c.circle(951,335,28,stroke=1,fill=0)
        self.text(c,930,327,'0.8' if name=='threat' else '8.4',25,b.CYAN);self.text(c,891,284,'연장 0.5 / 3초',15)
        self.text(c,1074,365,'다음 · 망치 견제',17);a.paint(c,'R1-ICONS','strike',1074,321,30,30);self.text(c,1112,328,'직접 피해 12',17);self.text(c,1074,294,'순서 예고',16)

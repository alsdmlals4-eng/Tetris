...
func test_battle_music_follows_modal_and_page_without_gameplay_changes():
    screen.session=screen.Session.new("STANDARD",42,"music-boundary")
    screen._reset_view()
    screen._show_page("battle")
    assert_true(screen.audio.background.playing)
    screen.open_details()
    assert_true(screen.audio.background.stream_paused)
    screen.close_details()
    assert_false(screen.audio.background.stream_paused)
    screen.pause_game()
    assert_true(screen.audio.background.stream_paused)
    screen.resume_game()
    assert_false(screen.audio.background.stream_paused)
    screen._show_page("main")
    assert_false(screen.audio.background.playing)

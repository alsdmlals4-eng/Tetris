...
Reader PR112 merged/read back at3f83fc03df24467f4819a9a6472878441ec94c53; exact6b79ee2 had2CI SUCCESS, threads0. New52page reader retains original37 pages and exposes actual implementation gaps. Next WG06 plan: REUSE existing actual-command expedition driver and screen geometry tests in a test-only native flow matrix. Alternatives manual-only repetitions (weak repeatability), engine-side public-command probe (ADOPT), new external UI automation stack (REJECT duplicate setup). Matrix will cover actual route completion, checkpoint restored paused with identical ETA/identity,125/100% font and1280/1920 window layout. It must not write normal user save/options, inject HP/rewards, enter runtime exports, or label synthetic commands as human/device input. Test helper presence/failure reporting first, then actual native execution and screenshot inspection. Keep human reading/fun/physical controller distinct.

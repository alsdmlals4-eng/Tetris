...
- Status: `USER_DIRECTED / PHASE 2 IMPLEMENTED_MACHINE_VERIFIED / RUNTIME_AND_HUMAN_EVIDENCE_PENDING`
- Date: 2026-08-28
...
This is `IMPLEMENTED_MACHINE_VERIFIED_PENDING_RUNTIME_AND_HUMAN_EVIDENCE`. Current exact-head evidence covers the schema/content boundary, target-separated time owners, the player-facing resolver and the category-only surface:
...
- `ProductionCombatRuntime` passes the full frame delta to the enemy scheduler, routes LINE through the reserve only while simulation is active, and gives the Skill resolver both narrow owners. A confirmed time effect records its target, changed value and unchanged domain for the battle surface.
- `src/production/ui/production_battle.gd` binds exactly `ATK / DEF / SUP`, one `SkillPreview`, `CONFIRM`, and `CANCEL`; `TierGrid` and manual Tier selection are absent.
- `src/production/skill/production_skill_session.gd` resolves from opening Combo, holds a C5 when affordable, falls back only to the highest MP-feasible lower Stage, preflights all effects before resource spend, and restores resources/effect owners on execution failure.

This implementation is automated-test evidence only. It does not prove target-device runtime behavior, balance, usability or Human/player understanding. Obtain target-resolution and first-exposure evidence before a usability/pass claim.
...
- The removed manual Tier 1–6 bridge remains historical provenance only; category-only selection is the active runtime path.

...
| `CNT-COMBO-RESOLVED-TECHNIQUES` | ATK/DEF/SUP resolved from the player’s current Combo Stage 1–10. | Category/preview/confirm, bounded fallback and the lower-Combo content direction are current design. The proposed C1–C10 matrix awaits user review; existing 18-entry manual Tier 1–6 seed is legacy, and data/resolver/time-control implementation remain required. |
...
| 2026-08-28 | Locked intentional lower-Combo casting and target-separated player board-time / enemy current-ETA semantics; recorded a proposed C1–C10 content matrix and the current implementation gap. | `TETRIS-SKILL-042 / ISSUE-80 / USER_REVIEW_REQUIRED / DOCUMENTED_NOT_IMPLEMENTED` |

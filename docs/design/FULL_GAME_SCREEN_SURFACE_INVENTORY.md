...
| `TETRIS-SCREEN-006` | E · Battle briefing / first-session launch | Planned · P0 | “Where am I, why is this Gatebreaker a threat, and what rules will govern my choices?” Read the immediate Vanguard / Frontier Gate / Gatebreaker context, then the full pre-Deploy LINE/CHAIN/Skill rule section; choose Deploy only after that readable boundary. | Title/direct first-session entry → Battle or back. | `PLANNED_GAME_SURFACE`; no separate briefing scene. `GAP_BLOCKING_FOR_INTENDED_FIRST_SESSION`, while current direct-entry runtime remains valid evidence. | Briefing layout with enemy, immediate threat, full readable rule region, and Deploy focus; reuse `TETRIS-SREF-003` as planning-only reference. |
| `TETRIS-SCREEN-007` | D/E · Continuous battle / embedded tutorial | Runtime · P0 | “Can I apply the rules I read before deployment while the ETA resolves?” Practice Telegraph/ETA, LINE/MP (Single/Double/Triple/Four = 10/22/36/52; hard cap 60), then the pre-disclosed CHAIN orthogonal straight-3+ H/V/diagonal rule: Combo +1 and `line total − 3 + current Combo = MP recovery` (Combo cap 10). Verify that an invalid swap reverts or may spend fixed **1 MP** to remain for setup, and either resets Combo. Decide whether to spend Combo on Skill now or retain it for later CHAIN MP; make one unforced response. | Briefing/direct entry → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`; tutorial extension, diagonal/MP-lock/Combo-reset/CHAIN-MP runtime, and MP/Combo-cap runtime are planned. `COVERED_EXISTING_FOR_CORE_RUNTIME`; intended first-session learning remains `NOT_RUN`. | Godot 960×540 capture plus first-exposure receipt; approved composition reference is not a tutorial PASS. |

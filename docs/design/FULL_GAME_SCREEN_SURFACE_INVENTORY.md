...
The intended first session is `TITLE → BATTLE_BRIEFING → SHORT_GUIDED_LIVE_PRACTICE → CONTINUOUS_BATTLE`: the briefing gives the minimum verified world/threat context and, on the first intended session only, requires the full rule region to reach its end or an equivalent accessible review action before Deploy unlocks. Deploy begins the real continuous clock. The first authored Telegraph/ETA is deliberately sufficient for the guided actions and nonterminal until the first explicit CONFIRM, then the short practice hands off to normal authored pressure in the same encounter. Later visits permit immediate Deploy and keep the rules re-openable. The implemented vertical slice now begins at `BATTLE_BRIEFING`; title, setup, route, result reward and workshop arrows remain planned. A popup is recorded as a separate surface only when it changes player decision, focus, or input ownership.

## Approved first-session onboarding · `TETRIS-ONBOARDING-037`

`TETRIS-SCREEN-006` is implemented for the **intended first session**. It uses only the current Vanguard / Frontier Gate / Gatebreaker / immediate-threat relationship. On that first visit, the complete rule region must reach its end before explicit Deploy enables and the CORE-029 clock can start; later visits may Deploy immediately and re-open the same rules from Battle. `TETRIS-SCREEN-007` then gives a short **safe live authored opening**: Current Telegraph/ETA genuinely runs from Deploy, but the first authored ETA is sufficient for practice and cannot force a terminal result before the first explicit CONFIRM. It practices LINE/MP → CHAIN/Combo → full Skill pause → category-resolved preview → explicit CONFIRM, then continues in the same encounter with normal authored pressure. CHAIN reads a straight horizontal, vertical, or diagonal 3+ run; an invalid swap restores by default and can optionally spend MP to remain as future Combo setup. These are machine-verified implementation facts; target-resolution and first-exposure evidence remain pending. The detailed rules and evidence gate live in `FIRST_SESSION_ONBOARDING_CONTRACT.md`, `CHAIN_COMBO_MP_CONTRACT.md` and `COMBO_RESOLVED_SKILL_CONTRACT.md`.
...
| `TETRIS-SCREEN-006` | E · Battle briefing / first-session launch | Implemented / machine verified · P0 | “Where am I, why is this Gatebreaker a threat, and what rules will govern my choices?” On the first intended session only, read the immediate Vanguard / Frontier Gate / Gatebreaker context and complete the full pre-Deploy LINE/CHAIN/Skill rule region before Deploy enables; later sessions may Deploy immediately and re-open the rules. | Direct first-session entry → Battle or reference close. | `GAME_RUNTIME`: `scenes/production/battle_briefing.tscn`; full rules gate, one-bit local completion and Deploy handoff are automated-verified. Target-device and Human evidence remain pending. | Godot target-resolution capture plus first-exposure receipt; planning board is not a tutorial PASS. |
| `TETRIS-SCREEN-007` | D/E · Continuous battle / embedded tutorial | Implemented / machine verified · P0 | “Can I verify the rules under a real live threat, then continue the same encounter?” A short **safe live opening** begins the actual 45-second ETA from Deploy. Its first authored Telegraph cannot force a terminal result before the first explicit CONFIRM, then hands off to normal authored pressure in the same encounter. MP lock remains optional; it never makes the tutorial longer by becoming mandatory. | Briefing → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`; CHAIN diagonal/MP-lock/Combo-reset/per-wave-MP, category Skill preview/fallback/atomic confirm and first-session handoff are machine-verified. Human learning remains `NOT_RUN`. | Godot target-resolution capture plus first-exposure receipt; planning board is not a tutorial PASS. |
| `TETRIS-SCREEN-008` | E · Tactical Skill overlay | Implemented / machine verified · P0 | “Which category answers this current threat, and is spending Combo now worth it?” Select ATK/DEF/SUP → inspect one current-Combo Stage/MP/fallback preview → CONFIRM or cancel. | Battle `K`/Skill → Battle. | `GAME_RUNTIME`: `SkillFrame`, one `SkillPreview`, `ConfirmButton`, `CancelButton` and target-separated timing feedback. Human comprehension is unverified. | Target-resolution tactical-pause capture including frozen threat/puzzle context, category preview and shortage fallback. |

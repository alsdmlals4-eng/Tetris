...
- Status: `CURRENT PLANNING INVENTORY / Issues #49 and #54`
- Authority: user-supplied screen-first inventory, `TETRIS-CORE-029`, `TETRIS-VISUAL-028`, `TETRIS-IMAGE-030`, `TETRIS-ONBOARDING-037`, and actual merged `main`.
...
`TETRIS-SCREEN-006` is now required for the **intended first session**, while remaining a planned rather than implemented surface. It uses only the current Vanguard / Frontier Gate / Gatebreaker / immediate-threat relationship, then waits for an explicit Deploy action before the CORE-029 clock starts. `TETRIS-SCREEN-007` teaches Current Telegraph + ETA → LINE/Energy → CHAIN/Stock/Tier → full Skill pause and explicit USE in that order, ending with one unforced response. The detailed rules and evidence gate live in `FIRST_SESSION_ONBOARDING_CONTRACT.md`.
...
| `TETRIS-SCREEN-007` | D/E · Continuous battle / embedded tutorial | Runtime · P0 | “Where should my attention go before the ETA resolves?” Learn Telegraph/ETA, LINE/Energy, CHAIN/Stock/Tier, then deliberate Skill USE; make one unforced response. | Briefing/direct entry → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`; tutorial extension is planned. `COVERED_EXISTING_FOR_CORE_RUNTIME`; intended first-session learning remains `NOT_RUN`. | Godot 960×540 capture plus first-exposure receipt; approved composition reference is not a tutorial PASS. |

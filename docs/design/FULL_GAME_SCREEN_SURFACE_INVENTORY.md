...
- Status: `CURRENT PLANNING INVENTORY / Issue #49`
- Authority: user-supplied screen-first inventory, `TETRIS-CORE-029`, `TETRIS-VISUAL-028`, `TETRIS-IMAGE-030`, and actual merged `main`.
...
The current direct-entry vertical slice begins at `CONTINUOUS_BATTLE`; all earlier/later arrows are planned, not implemented. A popup is recorded as a separate surface only when it changes player decision, focus, or input ownership.
...
| `TETRIS-SCREEN-006` | E · Battle briefing | Planned · P1 | “What is the threat and what can I prepare?” Inspect Gatebreaker phase, rewards, constraints; launch. | Route → Battle or back. | `PLANNED_GAME_SURFACE`; no separate briefing scene. `GAP_NONBLOCKING_FOR_DIRECT_ENTRY_SLICE`. | Briefing layout with enemy, risk, route and launch focus. |
| `TETRIS-SCREEN-007` | D/E · Continuous battle | Runtime · P0 | “Where should my attention go before the ETA resolves?” Control LINE/CHAIN, read telegraph/resources, commit a skill. | Briefing/direct entry → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`. `COVERED_EXISTING`; human readability remains `NOT_RUN`. | Godot 960×540 capture; approved composition reference. |
...
| `TETRIS-SCREEN-012` | G · Codex / archive / tutorial / controls | Planned explanatory · P1 | “How do LINE, CHAIN, ETA and tactical pause work?” Search/filter/read and return. | Any player request → prior surface. | `PLAYER_FACING_EXPLANATORY`; in-scene LINE control rail exists, dedicated help is absent. `GAP_NONBLOCKING`. | Manual/article layout wireframe with keyboard and gamepad legends. |
...
1. Create whole-screen planning references for `002 Title`, `005 Route`, `006 Briefing`, `010 Result`, and `012 Manual` as `PLANNED_GAME_SURFACE` candidates.
2. Convert the selected reference into a Godot screen contract before generating any runtime bitmap.
3. Implement and validate the smallest player flow extension one screen at a time.

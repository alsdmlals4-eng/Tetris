...
| `TETRIS-SCREEN-007` | D/E · Continuous battle / embedded tutorial | Runtime + planned correction · P0 | “Can I verify the rules under a real live threat, then continue the same encounter?” A short **safe live opening** begins the actual ETA from Deploy. Its first authored Telegraph has sufficient room for Telegraph/ETA, LINE/MP (Single/Double/Triple/Four = 10/22/36/52; hard cap 60), the pre-disclosed CHAIN orthogonal straight-3+ H/V/diagonal rule, category-resolved Skill preview and one explicit CONFIRM; it cannot force a terminal result before that CONFIRM. It then hands off to normal authored pressure and an unforced response in the same encounter. MP lock remains optional; it never makes the tutorial longer by becoming mandatory. | Briefing/direct entry → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`; CHAIN diagonal/MP-lock/Combo-reset/per-wave-MP and MP/Combo-cap runtime are machine-verified. Tutorial extension, category-resolved Skill and fallback remain planned. `COVERED_EXISTING_FOR_CORE_RUNTIME`; intended first-session learning remains `NOT_RUN`. | Godot target-resolution capture plus first-exposure receipt; planning board is not a tutorial PASS. |
| `TETRIS-SCREEN-008` | E · Tactical Skill overlay | Runtime + planned correction · P0 | “Which category answers this current threat, and is spending Combo now worth it?” Select ATK/DEF/SUP → inspect one current-Combo Stage/MP/fallback preview → CONFIRM or cancel. | Battle `K`/Skill → Battle. | Actual `GAME_RUNTIME`: `SkillFrame`, legacy `TierGrid`, `UseButton`; `TETRIS-SKILL-039` replaces that selection grammar but is `DOCUMENTED_NOT_IMPLEMENTED`. Human comprehension is unverified. | Target-resolution tactical-pause capture including frozen threat/puzzle context, category preview and shortage fallback. |

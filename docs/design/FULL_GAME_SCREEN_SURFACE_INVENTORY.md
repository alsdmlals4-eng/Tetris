...
| `TETRIS-SCREEN-002` | A · Title / main menu | Runtime current-worktree · P1 | “Deploy to the Frontier Gate or leave the current slice?” Start the fixed first encounter. | Boot → Battle Briefing. | `GAME_RUNTIME`: `scenes/production/title.tscn`; the user-locked **FRACTURE FRONTIER** raster logo is bound to the visible `TitleLogo` consumer and the duplicate label is hidden. `AUTOMATED_SCENE_VERIFIED_ON_CURRENT_WORKTREE`. | Godot target-resolution capture and Human readability remain required; scene wiring is not visual evidence. |
...
| `TETRIS-SCREEN-016` | Release · logo / icon / capsules / screenshots / key art | Planned distribution · P1 | “What is this game and why should I enter?” Store visitor reads identity and receives truthful gameplay proof. | Platform listing → store/game entry. | `PRODUCT_DISTRIBUTION`; **FRACTURE FRONTIER** title art is user-locked and runtime-bound, but platform target, commercial-rights/package review, capture set and store-specific output remain unresolved. `BLOCKED_UNVERIFIED_FOR_FINAL_ART`. | Store-spec-specific final files only after platform/rights/capture gate. |

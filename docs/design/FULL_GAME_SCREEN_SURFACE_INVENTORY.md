...
| `TETRIS-SCREEN-007` | D/E · Continuous battle / embedded tutorial | Runtime · P0 | “Where should my attention go before the ETA resolves?” Learn Telegraph/ETA, LINE/MP, CHAIN straight-3+ match and Combo/Tier, then deliberate Skill USE; an invalid swap reverts or may spend MP to remain for setup. Make one unforced response. | Briefing/direct entry → Skill, terminal result. | `GAME_RUNTIME`: `scenes/production/battle.tscn`; tutorial extension and diagonal/MP-lock runtime are planned. `COVERED_EXISTING_FOR_CORE_RUNTIME`; intended first-session learning remains `NOT_RUN`. | Godot 960×540 capture plus first-exposure receipt; approved composition reference is not a tutorial PASS. |

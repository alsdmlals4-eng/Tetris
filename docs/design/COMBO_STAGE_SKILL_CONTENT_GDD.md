...
- The active runtime now implements the user-approved Phase 2 mechanism as a **bounded stored opportunity reserve**: the existing temporary Skill executor can add displayed seconds to a `12.0`-second maximum reserve; while continuous simulation is running and `LINE` is the active workspace, the reserve supplies `0.0` only to LINE gravity/lock advancement while LINE input stays enabled. It consumes no reserve in `CHAIN`, during Skill/manual pause, or after terminal state; it does not change the scheduler's full real delta. The category-resolved preview/CONFIRM transaction and player-facing result presentation remain separate unimplemented work; this is not an existing Haste translation.
...
| --- | --- | --- | --- |
| Category → resolved preview → explicit confirm | `ProductionSkillSession` resolves the current Combo, previews without mutation, and atomically commits or restores all mutable owners. | `IMPLEMENTED_MACHINE_VERIFIED` | Target-device and Human readability validation remain required. |
| Enemy current-ETA acceleration/deceleration | `EnemyActionScheduler` owns `_remaining_seconds`, current action id and a commit boundary. | `IMPLEMENTED_MACHINE_VERIFIED` | Exact-action-id adjustment rejects Next/committed targets, clamps at the scheduler commit boundary and supports same-action rollback only. |
| Player board-play acceleration/deceleration | `PlayerBoardOpportunityState` owns a 12-second reserve; runtime gates only active LINE gravity/lock with it. | `IMPLEMENTED_MACHINE_VERIFIED` | Preserve LINE input, leave scheduler full-delta advancement unchanged, and consume nothing in CHAIN, pause or terminal state. |
| Existing data-driven effects | JSON definitions use the finite effect vocabulary; the resolver preflights selected variants and checkpoints only their actual mutable owners. | `IMPLEMENTED_MACHINE_VERIFIED` | Do not create per-skill scripts; retain the data-driven resolver boundary. |
...
| Current Stage 1–10 player promise | The runtime catalog validates C1–C10 and `ProductionCombatState` caps Combo at 10; the scene/session use one current-Combo category preview, bounded fallback and explicit CONFIRM. | `IMPLEMENTED_MACHINE_VERIFIED` | Balance, target-device and Human validation remain required. |
...
| 4 — false runtime claim | Could the legacy multiplier or status system make an authored effect appear to work when it is skipped/unwired? | The multiplier conflict remains recorded. The two time primitives now have resolver/runtime tests, but the final category-only preview/CONFIRM transaction is still not implemented. |
...
| 2026-09-01 | Implemented the narrow reserve and exact-current-ETA scheduler owners, then wired their approved effect primitives through the temporary Skill executor with deterministic tests. | `IMPLEMENTED_MACHINE_VERIFIED / CATEGORY_CONFIRM_FLOW_REMAINS_PENDING` |

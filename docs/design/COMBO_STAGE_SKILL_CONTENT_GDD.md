...
| Existing data-driven effects | JSON definitions use the finite effect vocabulary and the temporary executor now routes only the two approved time owners. | `PARTIAL` | Do not create per-skill scripts; replace the temporary manual selection with final category preview/CONFIRM and atomic rollback next. |
...
| Current Stage 1–10 player promise | The runtime catalog validates C1–C10 and `ProductionCombatState` caps Combo at 10; the scene/session still exposes a temporary manual Tier 1–6 bridge that selects C1–C6 directly. | `PARTIALLY_IMPLEMENTED_DATA_AND_TIME_PRIMITIVES` | Replace the bridge with the deterministic current-Combo category preview, MP-shortage fallback and final explicit CONFIRM transaction. |

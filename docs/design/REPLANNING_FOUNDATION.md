...
- **2026-09-20 resource-choice continuation:** user approved selectable LINE/SWAP resource production with fixed falling-pair automatic skills, and explicitly approved selective PR118 integration onto fresh main while preserving that PR. Current bounded owner: [R3 specification current addition](R3_FALLING_CHAIN_AND_DISRUPTION_SPEC.md), [execution receipt](../operations/RESOURCE_CHOICE_EXECUTION.json), [verification/readback](../validation/resource-choice-20260920/README.md). New playable entry is `scenes/replanned_r3/resource_choice.tscn`; production default/R2/old saves/plugins are preserved. Trial SWAP normalization is not measured balance or HUMAN approval. Merge status is live GitHub evidence; do not equate the working branch with main.

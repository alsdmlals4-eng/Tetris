...
- **2026-09-20 bonus-assist approved follow-up:** [R3 latest rules](R3_FALLING_CHAIN_AND_DISRUPTION_SPEC.md) and [execution receipt](../operations/BONUS_ASSIST_EXECUTION.json) own all-enemy destruction, cap10 overflow bonus, paid stable-boundary corrections/emergency effects, stronger six-tier finishers and short practice. Actual entry remains `scenes/replanned_r3/resource_choice.tscn`, now opt-in assist successor with isolated `user://bonus_assist/` saves. Legacy/default/campaign entries are preserved, not silently migrated. Implementation/runtime verification active; review/publication/main readback tracked in the receipt. New human balance/fun and release remain NOT_RUN.

...
- **2026-09-20 starter-finisher approved continuation:** user feedback supersedes manual category selection/per-wave casting in the resource-choice entry. First cleared symbol determines one end-of-chain effect; longer symmetric action presentation freezes both puzzles and ETA. [Current R3 specification](R3_FALLING_CHAIN_AND_DISRUPTION_SPEC.md) owns exact tie-break, aggregate effect, time cap, impact and save boundaries; [execution receipt](../operations/STARTER_FINISHER_EXECUTION.json) owns progress. Implementation/verification IN_PROGRESS, not merged or HUMAN-approved. Existing production/R2/legacy R3 saves and assets remain protected.

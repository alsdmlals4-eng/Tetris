...
- **2026-09-21 short expedition + counter guard IN_PROGRESS:** approved current slice connects latest selectable-resource/first-symbol-finisher combat to the existing three-battle route and carry supplies. DEF now additionally grants T charges: residual HP damage -50%, reflecting prevented damage once at enemy impact. [Rules](R3_FALLING_CHAIN_AND_DISRUPTION_SPEC.md#short-expedition-counter-guard), [execution](../operations/SHORT_EXPEDITION_EXECUTION.json), [evidence](../validation/short-expedition-20260921/README.md). New entry `scenes/replanned_r3/short_expedition.tscn`; current standalone `resource_choice.tscn` opts into counter successor. Isolated `user://short_expedition/` and `user://counter_guard/`; previous Mastery entry `mastery_legacy.tscn` preserves its own saves. Production/default and old rule paths remain protected. Native fixtures and tests are not HUMAN balance/fun; final verification/merge pending.

...
**Current R3 successor (2026-09-21):** read the newest foundation entry and [R3 rules](R3_FALLING_CHAIN_AND_DISRUPTION_SPEC.md) before the dated migration readback below. Selected R3 work was integrated through subsequent main PRs; the old Drafts themselves remain read-only. `scenes/replanned_r3/short_expedition.tscn` is the latest short-run entry, `resource_choice.tscn` its single-battle/practice entry, and `mastery_legacy.tscn` the explicitly preserved previous-save entry. Current verification and remaining gates: [short expedition evidence](../validation/short-expedition-20260921/README.md). The production tables and historical PDFs below do not own these successor rules.

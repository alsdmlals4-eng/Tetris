...
| --- | --- | --- | --- | --- | --- | --- |
| `TETRIS-IMG-033` · Vanguard Combat Cutout v1 | `IMG-P0-002` | `res://assets/production/characters/vanguard_combat_cutout_v1.png` | `scenes/production/battle.tscn` → `MainRow/CombatColumn/CombatStage/VanguardReference`; same source is atlas-cropped at `ResourceRow/VanguardPortrait` | stage `TextureRect.texture: Texture2D`; resource `AtlasTexture` crop | transparent alpha; vertical full body, sword and shield retained; max source dimension 1536 px; full-height, aspect-centered stage slot; face-and-shoulder crop has a 96×96 resource safe slot | `SOURCE_ASSET_CANDIDATE` · `RUNTIME_INTEGRATION: IMPLEMENTED_ON_BRANCH` · `RUNTIME_VERIFICATION: CURRENT_WORKTREE_RUNTIME_CAPTURED_1280X720_NOT_EXACT_COMMITTED_HEAD` |
| `TETRIS-IMG-034` · Gatebreaker Combat Cutout v1 | `IMG-P0-003` | `res://assets/production/bosses/gatebreaker_combat_cutout_v1.png` | `scenes/production/battle.tscn` → `MainRow/CombatColumn/CombatStage/GatebreakerReference` | `TextureRect.texture: AtlasTexture` sourced from the approved cutout | transparent alpha; declared upper-body atlas crop retains ram-arm, face and visible Rift Core; max source dimension 1536 px; covered stage slot with `0.06` left anchor and `-0.14–1.14` vertical overscan | `SOURCE_ASSET_CANDIDATE` · `RUNTIME_INTEGRATION: IMPLEMENTED_ON_BRANCH` · `RUNTIME_VERIFICATION: CURRENT_WORKTREE_RUNTIME_CAPTURED_1280X720_NOT_EXACT_COMMITTED_HEAD` |

The current 50/50 composition binds the Vanguard cutout as a pointer-transparent, aspect-preserving direct child of `CombatStage` and composes the approved Gatebreaker source as an upper-body `AtlasTexture` within the same named consumer. The compact Vanguard stage cutout occupies the left `0.0–0.32` foreground band, while the Gatebreaker begins at `0.06`, overscans vertically `-0.14–1.14`, and uses covered scaling to own the dominant right-side silhouette. A separate `ResourceRow/VanguardPortrait` composes the approved Vanguard source into a readable 104×104 face-and-shoulder state-strip slot without competing with the boss stage. This preserves the board, forecast, resource, and skill regions outside the art bounds. Automated scene construction proves these slots load; it does not establish Human readability or final commercial-art approval.

...
| Camera/framing | One large active puzzle surface plus persistent combat/threat context. The current user-directed runtime target is an equal 50/50 puzzle/combat composition; the Gatebreaker owns the dominant combat-stage silhouette while a separate compact Vanguard portrait preserves player identity. |

...
Checkpoint follow-through: explicit paused save/load now uses the existing R3-only writer; missing/corrupt load preserves the live session, restored sessions stay paused, and cosmetic history is not replayed. Fractional simulation time freezes during pause. Pause menu exposes reduced motion (static impact pose without entrance/exit translation). Native run9 saved/read back an isolated `replanned_r3_tests/native-save-31561` record, confirmed pause=true/replay empty, and `pause-save.png` was visually inspected. The test writer was then restored to normal R3 configuration without writing a normal save. Full corrected regression **487/487,7197assertions,80scripts**,88.424s,exit0,stderr empty (`r3-checkpoint-full.log/.err`). Two checkpoint reviews cover source/storage identity/failure preservation and rendered menu/paused time/replay boundaries; no new blocking finding in this bounded follow-through. User options persistence, route/menu and full-game gates still remain.
